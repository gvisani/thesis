...
We describe here our general framework for building neural networks that are equivariant to 3D rotations 
...
to $\mathfrak{G}$.
See Figure~\ref{fig:invariance_and_equivariance} for a visual reference of these concepts.

\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/invariance_and_equivariance.pdf}
    \figcaption{Invariance and Equivariance to 3D rotations, explained using a water molecule.}{
        The mass of a water molecule does not change if the molecule is rotated - it is said to be \textit{invariant}
        to rotations of the molecule.
        Instead, its dipole moment rotates alongside the molecule - it is said to be \textit{equivariant}
        to rotations of the molecule.
        See text for a more formal mathematical definition.
        Slide credit: Michael Pun.
        }
   \label{fig:invariance_and_equivariance}
\end{figure}
...
\begin{figure}[b]
    ...
    \figcaption{Visualization of the Holograhic Encoding considered in this thesis.}{}
    \medskip
    \small
    \justifying
